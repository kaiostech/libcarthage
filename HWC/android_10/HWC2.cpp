...
Device::Device(const std::string& serviceName)
    : mComposer(std::make_unique<Hwc2::impl::Composer>(serviceName)) {
  loadCapabilities();
}
...
void Device::registerCallback(ComposerCallback* callback, int32_t sequenceId) {
  if (mRegisteredCallback) {
    ALOGW(
        "Callback already registered. Ignored extra registration "
        "attempt.");
    return;
  }
...
Error Device::getDisplayIdentificationData(
    hwc2_display_t hwcDisplayId, uint8_t* outPort,
...
Display::~Display() {
  ...
  if (mType == DisplayType::Virtual) {
    ...
    ALOGE_IF(error != Error::None,
             "destroyVirtualDisplay(%" PRIu64 ") failed: %s (%d)", mId,
    ...
    if (error != Error::None) {
      ALOGE("~Display: Failed to disable vsync for display %" PRIu64
            ": %s (%d)",
            mId, to_string(error).c_str(), static_cast<int32_t>(error));
    }
  }
}
...
Error Display::createLayer(HWC2::Layer** outLayer) {
  ...
  auto layer =
      std::make_unique<impl::Layer>(mComposer, mCapabilities, mId, layerId);
  *outLayer = layer.get();
...
Error Display::getChangedCompositionTypes(
    std::unordered_map<HWC2::Layer*, Composition>* outTypes) {
  std::vector<Hwc2::Layer> layerIds;
  std::vector<Hwc2::IComposerClient::Composition> types;
  auto intError = mComposer.getChangedCompositionTypes(mId, &layerIds, &types);
  ...
  for (uint32_t element = 0; element < numElements; ++element) {
    ...
    if (layer) {
      ...
      ALOGV("getChangedCompositionTypes: adding %" PRIu64 " %s", layer->getId(),
            to_string(type).c_str());
      outTypes->emplace(layer, type);
    } else {
      ALOGE("getChangedCompositionTypes: invalid layer %" PRIu64
            " found"
            " on display %" PRIu64,
            layerIds[element], mId);
    }
  }
...
int32_t Display::getSupportedPerFrameMetadata() const {
  ...
  std::vector<Hwc2::PerFrameMetadataKey> tmpKeys =
      mComposer.getPerFrameMetadataKeys(mId);
  std::set<Hwc2::PerFrameMetadataKey> keys(tmpKeys.begin(), tmpKeys.end());

  // Check whether a specific metadata type is supported. A metadata type is
  // considered supported if and only if all required fields are supported.
  ...
  // CTA861_3
  if (hasMetadataKey(keys,
                     Hwc2::PerFrameMetadataKey::MAX_CONTENT_LIGHT_LEVEL) &&
      hasMetadataKey(
          keys, Hwc2::PerFrameMetadataKey::MAX_FRAME_AVERAGE_LIGHT_LEVEL)) {
...
Error Display::getRenderIntents(
    ColorMode colorMode, std::vector<RenderIntent>* outRenderIntents) const {
  auto intError = mComposer.getRenderIntents(mId, colorMode, outRenderIntents);
  return static_cast<Error>(intError);
}

Error Display::getDataspaceSaturationMatrix(Dataspace dataspace,
                                            android::mat4* outMatrix) {
  auto intError = mComposer.getDataspaceSaturationMatrix(dataspace, outMatrix);
  return static_cast<Error>(intError);
}

std::vector<std::shared_ptr<const Display::Config>> Display::getConfigs()
    const {
...
Error Display::getRequests(
    HWC2::DisplayRequest* outDisplayRequests,
    std::unordered_map<HWC2::Layer*, LayerRequest>* outLayerRequests) {
  uint32_t intDisplayRequests;
  std::vector<Hwc2::Layer> layerIds;
  std::vector<uint32_t> layerRequests;
  auto intError = mComposer.getDisplayRequests(mId, &intDisplayRequests,
                                               &layerIds, &layerRequests);
  uint32_t numElements = layerIds.size();
  auto error = static_cast<Error>(intError);
  if (error != Error::None) {
    return error;
  ...
  *outDisplayRequests = static_cast<DisplayRequest>(intDisplayRequests);
  ...
  for (uint32_t element = 0; element < numElements; ++element) {
    auto layer = getLayerById(layerIds[element]);
    if (layer) {
      auto layerRequest = static_cast<LayerRequest>(layerRequests[element]);
      outLayerRequests->emplace(layer, layerRequest);
    } else {
      ALOGE("getRequests: invalid layer %" PRIu64 " found on display %" PRIu64,
            layerIds[element], mId);
    }
  }
...
Error Display::getHdrCapabilities(HdrCapabilities* outCapabilities) const {
  ...
  auto intError = mComposer.getHdrCapabilities(
      mId, &types, &maxLuminance, &maxAverageLuminance, &minLuminance);
  ...
  *outCapabilities = HdrCapabilities(std::move(types), maxLuminance,
                                     maxAverageLuminance, minLuminance);
  return Error::None;
}

Error Display::getDisplayedContentSamplingAttributes(
    PixelFormat* outFormat, Dataspace* outDataspace,
    uint8_t* outComponentMask) const {
  auto intError = mComposer.getDisplayedContentSamplingAttributes(
      mId, outFormat, outDataspace, outComponentMask);
  return static_cast<Error>(intError);
}

Error Display::setDisplayContentSamplingEnabled(bool enabled,
                                                uint8_t componentMask,
                                                uint64_t maxFrames) const {
  auto intError = mComposer.setDisplayContentSamplingEnabled(
      mId, enabled, componentMask, maxFrames);
  return static_cast<Error>(intError);
}

Error Display::getDisplayedContentSample(
    uint64_t maxFrames, uint64_t timestamp,
    android::DisplayedFrameStats* outStats) const {
  auto intError =
      mComposer.getDisplayedContentSample(mId, maxFrames, timestamp, outStats);
  return static_cast<Error>(intError);
}

Error Display::getReleaseFences(
    std::unordered_map<HWC2::Layer*, sp<Fence>>* outFences) const {
  std::vector<Hwc2::Layer> layerIds;
...
Error Display::setActiveConfig(const std::shared_ptr<const Config>& config) {
  if (config->getDisplayId() != mId) {
    ALOGE("setActiveConfig received config %u for the wrong display %" PRIu64
          " (expected %" PRIu64 ")",
...
Error Display::setColorTransform(const android::mat4& matrix,
  ...
  auto intError = mComposer.setColorTransform(
      mId, matrix.asArray(), static_cast<Hwc2::ColorTransform>(hint));
  return static_cast<Error>(intError);
}
...
Error Display::setPowerMode(PowerMode mode) {
  ...
  if (mode == PowerMode::On) {
    std::call_once(mDisplayCapabilityQueryFlag, [this]() {
      ...
      auto error = static_cast<Error>(
          mComposer.getDisplayCapabilities(mId, &tmpCapabilities));
      if (error == Error::None) {
        for (auto capability : tmpCapabilities) {
          mDisplayCapabilities.emplace(
              static_cast<DisplayCapability>(capability));
        }
      } else if (error == Error::Unsupported) {
        if (mCapabilities.count(Capability::SkipClientColorTransform)) {
          mDisplayCapabilities.emplace(
              DisplayCapability::SkipClientColorTransform);
        }
...
Error Display::presentOrValidate(uint32_t* outNumTypes,
...
int32_t Display::getAttribute(hwc2_config_t configId, Attribute attribute) {
  int32_t value = 0;
  auto intError = mComposer.getDisplayAttribute(
      mId, configId, static_cast<Hwc2::IComposerClient::Attribute>(attribute),
      &value);
  auto error = static_cast<Error>(intError);
  if (error != Error::None) {
    ALOGE("getDisplayAttribute(%" PRIu64 ", %u, %s) failed: %s (%d)", mId,
          configId, to_string(attribute).c_str(), to_string(error).c_str(),
          intError);
    return -1;
  }
  return value;
...
Error Layer::setBuffer(uint32_t slot, const sp<GraphicBuffer>& buffer,
  ...
  int32_t fenceFd = acquireFence->dup();
  auto intError =
      mComposer.setLayerBuffer(mDisplayId, mId, slot, buffer, fenceFd);
  return static_cast<Error>(intError);
}
...
Error Layer::setSurfaceDamage(const Region& damage) {
  ...
  if (damage.isRect() && damage.getBounds() == Rect::INVALID_RECT) {
    intError = mComposer.setLayerSurfaceDamage(
        mDisplayId, mId, std::vector<Hwc2::IComposerClient::Rect>());
  } else {
    size_t rectCount = 0;
...
Error Layer::setCompositionType(Composition type) {
  ...
  auto intError = mComposer.setLayerCompositionType(mDisplayId, mId, intType);
  return static_cast<Error>(intError);
}
...
Error Layer::setPerFrameMetadata(const int32_t supportedPerFrameMetadata,
  ...
  if (validTypes & HdrMetadata::SMPTE2086) {
    perFrameMetadatas.insert(
        perFrameMetadatas.end(),
  ...
  if (validTypes & HdrMetadata::HDR10PLUS) {
    ...
    Error setMetadataBlobsError =
        static_cast<Error>(mComposer.setLayerPerFrameMetadataBlobs(
            mDisplayId, mId, perFrameMetadataBlobs));
    if (error == Error::None) {
...
Error Layer::setDisplayFrame(const Rect& frame) {
  Hwc2::IComposerClient::Rect hwcRect{frame.left, frame.top, frame.right,
                                      frame.bottom};
...
Error Layer::setSidebandStream(const native_handle_t* stream) {
  if (mCapabilities.count(Capability::SidebandStream) == 0) {
    ALOGE(
        "Attempted to call setSidebandStream without checking that the "
...
Error Layer::setSourceCrop(const FloatRect& crop) {
  Hwc2::IComposerClient::FRect hwcRect{crop.left, crop.top, crop.right,
                                       crop.bottom};
...
Error Layer::setColorTransform(const android::mat4& matrix) {
  ...
  auto intError =
      mComposer.setLayerColorTransform(mDisplayId, mId, matrix.asArray());
