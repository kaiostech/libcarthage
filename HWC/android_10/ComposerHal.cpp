...
Error unwrapRet(Return<Error>& ret) { return unwrapRet(ret, kDefaultError); }
...
Composer::Composer(const std::string& serviceName)
  ...
  if (sp<IComposer> composer_2_3 = IComposer::castFrom(mComposer)) {
    ...
    mComposer->createClient([&](const auto& tmpError, const auto& tmpClient) {
      ...
      mClient = tmpClient;
      if (sp<V2_2::IComposer> composer_2_2 =
              V2_2::IComposer::castFrom(mComposer)) {
...
std::string Composer::dumpDebugInfo() {
  ...
  mComposer->dumpDebugInfo(
      [&](const auto& tmpInfo) { info = tmpInfo.c_str(); });
...
bool Composer::isRemote() { return mClient->isRemote(); }

void Composer::resetCommands() { mWriter.reset(); }

Error Composer::executeCommands() { return execute(); }
...
Error Composer::createVirtualDisplay(uint32_t width, uint32_t height,
  ...
  if (mClient_2_2) {
    mClient_2_2->createVirtualDisplay_2_2(
        width, height, static_cast<types::V1_1::PixelFormat>(*format),
        bufferSlotCount,
        [&](const auto& tmpError, const auto& tmpDisplay,
          ...
          *format = static_cast<types::V1_2::PixelFormat>(tmpFormat);
        });
  } else {
    mClient->createVirtualDisplay(
        width, height, static_cast<types::V1_0::PixelFormat>(*format),
        bufferSlotCount,
        [&](const auto& tmpError, const auto& tmpDisplay,
            const auto& tmpFormat) {
          error = tmpError;
...
Error Composer::getChangedCompositionTypes(
...
Error Composer::getColorModes(Display display,
  ...
  if (mClient_2_3) {
    mClient_2_3->getColorModes_2_3(
        display, [&](const auto& tmpError, const auto& tmpModes) {
          error = tmpError;
          if (error != Error::NONE) {
            return;
          }
...
Error Composer::getDisplayRequests(
    Display display, uint32_t* outDisplayRequestMask,
    std::vector<Layer>* outLayers,
    std::vector<uint32_t>* outLayerRequestMasks) {
  mReader.takeDisplayRequests(display, outDisplayRequestMask, outLayers,
...
Error Composer::getHdrCapabilities(Display display, std::vector<Hdr>* outTypes,
                                   float* outMaxLuminance,
                                   float* outMaxAverageLuminance,
                                   float* outMinLuminance) {
  Error error = kDefaultError;
  if (mClient_2_3) {
    mClient_2_3->getHdrCapabilities_2_3(
        display,
...
Error Composer::getReleaseFences(Display display, std::vector<Layer>* outLayers,
...
Error Composer::setClientTarget(
    Display display, uint32_t slot, const sp<GraphicBuffer>& target,
...
Error Composer::setColorMode(Display display, ColorMode mode,
  ...
  if (mClient_2_3) {
    ...
    ret = mClient_2_2->setColorMode_2_2(
        display, static_cast<types::V1_1::ColorMode>(mode), renderIntent);
  } else {
...
Error Composer::setPowerMode(Display display, IComposerClient::PowerMode mode) {
  ...
  if (mClient_2_2) {
    ...
    ret = mClient->setPowerMode(
        display, static_cast<V2_1::IComposerClient::PowerMode>(mode));
  }

  return unwrapRet(ret);
}

Error Composer::setVsyncEnabled(Display display,
...
Error Composer::setCursorPosition(Display display, Layer layer, int32_t x,
                                  int32_t y) {
  mWriter.selectDisplay(display);
...
Error Composer::setLayerBuffer(Display display, Layer layer, uint32_t slot,
...
Error Composer::setLayerSurfaceDamage(
...
Error Composer::setLayerPlaneAlpha(Display display, Layer layer, float alpha) {
  mWriter.selectDisplay(display);
  mWriter.selectLayer(layer);
...
Error Composer::setLayerVisibleRegion(
    Display display, Layer layer,
...
Error Composer::execute() {
  ...
  if (mClient_2_2) {
    ret = mClient_2_2->executeCommands_2_2(commandLength, commandHandles,
                                           hidl_callback);
  } else {
    ret =
        mClient->executeCommands(commandLength, commandHandles, hidl_callback);
  }
  ...
  if (error == Error::NONE) {
    ...
    for (const auto& cmdErr : commandErrors) {
      auto command = static_cast<IComposerClient::Command>(
          mWriter.getCommand(cmdErr.location));
...
Error Composer::setLayerPerFrameMetadata(
    Display display, Layer layer,
...
std::vector<IComposerClient::PerFrameMetadataKey>
Composer::getPerFrameMetadataKeys(Display display) {
  ...
  Error error = kDefaultError;
  if (mClient_2_3) {
    mClient_2_3->getPerFrameMetadataKeys_2_3(
        display, [&](const auto& tmpError, const auto& tmpKeys) {
          error = tmpError;
          if (error != Error::NONE) {
            ALOGW(
                "getPerFrameMetadataKeys failed "
                "with %d",
                tmpError);
            return;
          }
    ...
    mClient_2_2->getPerFrameMetadataKeys(display, [&](const auto& tmpError,
                                                      const auto& tmpKeys) {
      error = tmpError;
      if (error != Error::NONE) {
...
Error Composer::getRenderIntents(Display display, ColorMode colorMode,
  ...
  if (mClient_2_3) {
    mClient_2_3->getRenderIntents_2_3(display, colorMode,
                                      getRenderIntentsLambda);
  } else {
    mClient_2_2->getRenderIntents(
        display, static_cast<types::V1_1::ColorMode>(colorMode),
        getRenderIntentsLambda);
  }
...
Error Composer::getDataspaceSaturationMatrix(Dataspace dataspace,
                                             mat4* outMatrix) {
  if (!mClient_2_2) {
    *outMatrix = mat4();
    return Error::NONE;
  ...
  Error error = kDefaultError;
  mClient_2_2->getDataspaceSaturationMatrix(
      static_cast<types::V1_1::Dataspace>(dataspace),
...
Error Composer::getDisplayIdentificationData(Display display, uint8_t* outPort,
  ...
  Error error = kDefaultError;
  mClient_2_3->getDisplayIdentificationData(
      display,
      [&](const auto& tmpError, const auto& tmpPort, const auto& tmpData) {
        error = tmpError;
        if (error != Error::NONE) {
          return;
        }
...
Error Composer::setLayerColorTransform(Display display, Layer layer,
                                       const float* matrix) {
  if (!mClient_2_3) {
    return Error::UNSUPPORTED;
  }
...
Error Composer::getDisplayedContentSamplingAttributes(
    Display display, PixelFormat* outFormat, Dataspace* outDataspace,
  ...
  Error error = kDefaultError;
  mClient_2_3->getDisplayedContentSamplingAttributes(
      display, [&](const auto tmpError, const auto& tmpFormat,
                   const auto& tmpDataspace, const auto& tmpComponentMask) {
        error = tmpError;
        if (error == Error::NONE) {
          *outFormat = tmpFormat;
          *outDataspace = tmpDataspace;
          *outComponentMask = static_cast<uint8_t>(tmpComponentMask);
        }
      });
  return error;
}

Error Composer::getDisplayCapabilities(
    Display display, std::vector<DisplayCapability>* outCapabilities) {
  if (!mClient_2_3) {
    return Error::UNSUPPORTED;
  }
  Error error = kDefaultError;
  mClient_2_3->getDisplayCapabilities(
      display, [&](const auto& tmpError, const auto& tmpCapabilities) {
        error = tmpError;
        if (error != Error::NONE) {
          return;
        }
...
Error Composer::setDisplayContentSamplingEnabled(Display display, bool enabled,
  ...
  return mClient_2_3->setDisplayedContentSamplingEnabled(
      display, enable, componentMask, maxFrames);
}

Error Composer::getDisplayedContentSample(Display display, uint64_t maxFrames,
                                          uint64_t timestamp,
  ...
  Error error = kDefaultError;
  mClient_2_3->getDisplayedContentSample(
      display, maxFrames, timestamp,
      [&](const auto tmpError, auto tmpNumFrames, const auto& tmpSamples0,
          const auto& tmpSamples1, const auto& tmpSamples2,
          const auto& tmpSamples3) {
        error = tmpError;
        if (error == Error::NONE) {
...
CommandReader::~CommandReader() { resetData(); }
...
Error CommandReader::parse() {
  ...
  while (!isEmpty()) {
    auto command_2_1 =
        reinterpret_cast<V2_1::IComposerClient::Command*>(&command);
...
void CommandReader::takeDisplayRequests(
    Display display, uint32_t* outDisplayRequestMask,
    std::vector<Layer>* outLayers,
    std::vector<uint32_t>* outLayerRequestMasks) {
  auto found = mReturnData.find(display);
...
void CommandReader::takePresentOrValidateStage(Display display,
                                               uint32_t* state) {
  auto found = mReturnData.find(display);
  if (found == mReturnData.end()) {
    *state = -1;
